...
class EliteImpactResult(BaseModel):
    ...
    # Add all relevant fields from EliteImpactColumns as optional floats/strings
    delta_impact_raw: float = Field(default=0.0)
    gamma_impact_raw: float = Field(default=0.0)
    vega_impact_raw: float = Field(default=0.0)
    theta_impact_raw: float = Field(default=0.0)
    vanna_impact_raw: float = Field(default=0.0)
    vomma_impact_raw: float = Field(default=0.0)
    charm_impact_raw: float = Field(default=0.0)
    sdag_multiplicative: float = Field(default=0.0)
    sdag_directional: float = Field(default=0.0)
    sdag_weighted: float = Field(default=0.0)
    sdag_volatility_focused: float = Field(default=0.0)
    sdag_consensus: float = Field(default=0.0)
    dag_multiplicative: float = Field(default=0.0)
    dag_directional: float = Field(default=0.0)
    dag_weighted: float = Field(default=0.0)
    dag_volatility_focused: float = Field(default=0.0)
    dag_consensus: float = Field(default=0.0)
    strike_magnetism_index: float = Field(default=0.0)
    volatility_pressure_index: float = Field(default=0.0)
    flow_momentum_index: float = Field(default=0.0)
    institutional_flow_score: float = Field(default=0.0)
    regime_adjusted_gamma: float = Field(default=0.0)
    regime_adjusted_delta: float = Field(default=0.0)
    regime_adjusted_vega: float = Field(default=0.0)
    cross_exp_gamma_surface: float = Field(default=0.0)
    expiration_transition_factor: float = Field(default=0.0)
    flow_velocity_5m: float = Field(default=0.0)
    flow_velocity_15m: float = Field(default=0.0)
    flow_acceleration: float = Field(default=0.0)
    momentum_persistence: float = Field(default=0.0)
    market_regime: str = Field(default="unknown")
    flow_type: str = Field(default="unknown")
    volatility_regime: str = Field(default="unknown")
    elite_impact_score: float = Field(default=0.0)
